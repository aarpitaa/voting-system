 ...
 *
 * Author: Arpita Dev, Neha Bhatia, Stuti Arora
 */
...
public class Ballot {
    ...
    public Candidate getNextCandidate(){        
        //Iterate through ballots until valid candidate is found
        while (formattedBallot.size() > 0) {
            formattedBallot.remove(0);
            if(formattedBallot.size() > 0) {
                if (formattedBallot.get(0) != null) {
                    if(formattedBallot.get(0).isInElection()) {
                        return formattedBallot.get(0);
                    }
