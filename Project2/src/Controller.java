...
public class Controller {
  ...
  public static void main(String[] args) throws IOException {
    ...
    System.out.println(
      "<yourfilename>.csv: To count votes for Instant Runoff or Open Party List voting, enter the full filename with the \".csv\" extension. Please make sure that your file is inside the folder \"electionFiles\" in the \"src\" directory."
    );
    ...
    while (!userInput.equals("exit")) {
      if (userInput.equals("test")) {
        ...
        if (fileProcessor.setInputFile(userInput)) {
          ...
          } else if (electionType.getElectionType().equals("OPL")) {
            fileProcessor.readRemainingOPL(electionType);
          } else {
            fileProcessor.readRemainingMPO(electionType);
          }
        } else {
