...
public class OPLElectionTests {
  ...
  public void testTieBreakerOPL() {
   ...
   // both pR and pA have 4 votes each, and they should have around a 50% split in winning the last seat
   int sum_pR = 0;
   int sum_pA = 0;
   ...
   for(int i = 0; i < 100; i++){
      ...
      elec.runElection();
      Assert.assertEquals(1, pD.getPartySeatCount());
      if(pR.getPartySeatCount() == 2 && pA.getPartySeatCount() == 1){
         sum_pR++;
      }
      else if(pA.getPartySeatCount() == 2 && pR.getPartySeatCount() == 1){
         sum_pA++;
      }

      
   }
      Assert.assertEquals(100, sum_pA + sum_pR);

      System.out.println("Percentage of pR getting 2 seats: " + sum_pR);
  ...
  public OPLElection setUpElection(){
    ...
    pD.incrementVoteCount(); // pD has 3 votes

    b.incrementBallotCount(); // b has 1 vote
  ...
  // TODO : what if candidates have ties??
  // ties before second round - 10 ballots, 4 seats ?? 4, 4, 2
}
